...
class JobVacanciesSearchTest extends BrowserTestBase {
   ...
   */
  public function testJobVacanciesSearchFindsCorrectResults(): void {
    // Arrange.
    ...
    $assert->pageTextContains('First vacancy');
    $assert->pageTextContains('Second vacancy');
    $assert->pageTextContains('Third vacancy');
   ...
   * Test that the posts we expected are showing.
   */
  public function testJobVacanciesSearchFindsNoResults(): void {
    // Arrange.
    $closing_date = DrupalDateTime::createFromTimestamp(time());
    $closing_date->setTimezone(new \DateTimeZone('UTC'));
    $closing_date->modify('+2 weeks');
